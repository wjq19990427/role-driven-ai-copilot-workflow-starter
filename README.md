...
/wrap task-N           # 进入复盘模式，对照 task-N 审 codex/task-N 分支
```

`/execute` 和 `/wrap` 必须指定任务卡 ID（一步法）。也可两步法：先触发再说"做 task-N"。
...
└── docs/
    ...
    ├── WRAP_PROTOCOL.md          # Review 流程详细规范
    ├── api/
    │   └── _HOWTO.md               # L2 契约编写指南
    ├── tasks/
    │   └── _template.md            # 任务卡模板
...
记忆系统不是可选扩展，而是长期项目的标配。只有 `/wrap` 角色能写，确保单一写入源。
...
| 角色数 | 2（架构师 + 实现工） | 5（PM/Diagnose/Design/Execute/Wrap） |
| 核心产物 | 任务卡 | 角色定义 + 任务卡 |
